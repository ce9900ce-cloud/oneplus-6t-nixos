...
Push this repo to GitHub and enable Actions. A CI workflow will cross-compile
NixOS images for OnePlus 6T using QEMU binfmt emulation.

1. Push to GitHub (make the repo public or private)
2. Add a secret `CACHIX_AUTH_TOKEN` (optional, speeds up builds)
3. Go to Actions tab → "Build OnePlus 6T Images" → Run workflow
4. Download artifacts: `nixos-disk-images-fajita` + `nixos-uboot-fajita`

See `.github/workflows/build.yml` for the workflow definition.
